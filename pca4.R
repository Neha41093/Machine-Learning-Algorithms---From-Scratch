#Q4 -- PCA and clustering(Kmeans and EM) comparison
...
pca_ringnorm <- prcomp(ringnorm_data)
pca_ionosphere <- prcomp(ionosphere_data)

#scatter plots
attach(pca_ringnorm)
plot(x[,1],x[,2],main="Ringnorm data - PC1 and PC2",xlab="PC1",ylab="PC2")
attach(pca_ionosphere)
plot(x[,1],x[,2],main="Ionosphere data - PC1 and PC2",xlab="PC1",ylab="PC2")

#screeplots
screeplot(pca_ringnorm)
screeplot(pca_ionosphere)

#loadings
View(pca_ringnorm$rotation)
View(pca_ionosphere$rotation)

#Keeping 90% variance and updating datasets
ringnorm_data_old <- ringnorm_data
pca_ringnorm_comps <- which(cumsum(pca_ringnorm$sdev^2/sum(pca_ringnorm$sdev^2)) <= 0.90)
ringnorm_data <- pca_ringnorm$x[,pca_ringnorm_comps]

ionosphere_data_old <- ionosphere_data
pca_ionosphere_comps <- which(cumsum(pca_ionosphere$sdev^2/sum(pca_ionosphere$sdev^2)) <= 0.90)
ionosphere_data <- pca_ionosphere$x[,pca_ionosphere_comps]

#Running Ck and Gk with new data
#**********************Ringnorm dataset********************************
...
#Ionosphere reduced dataset
ionosphere_data_with_labels <- read.csv("ionosphere_data.csv",header=FALSE)
ionosphere_old_data <- as.matrix(ionosphere_data_with_labels[,1:34])
pca_ionosphere <- prcomp(ionosphere_old_data)
pca_ionosphere_comps <- which(cumsum(pca_ionosphere$sdev^2/sum(pca_ionosphere$sdev^2)) <= 0.90)
ionosphere_data <- pca_ionosphere$x[,pca_ionosphere_comps]
